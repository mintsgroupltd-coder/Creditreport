...
| `GET /api/reports/:id` | Full report: stats, accounts, events, alerts |
| `GET /api/reports/:id/dispute-text` | Plain-text dispute letter body, generated from whatever the analytics engine actually found on that report |
| `DELETE /api/reports/:id` | Delete a report (cascades to its accounts/events/alerts) |
| `GET /api/accounts/:id` | One account's detail plus its merged timeline (monthly history + dated events) |
...
- **`AlertList`** — renders `Alert` rows with a severity dot and a
  human label (`DOB_MISMATCH` → "Date of birth mismatch", etc).
- **`AccountTable`** — the accounts grid on the report page, links each
  row through to `AccountDetailPage`.
- **`AccountTimeline`** — vertical timeline combining monthly
  balance/status points and dated events into one sorted list.

Pages: `DashboardPage` (list + upload), `ReportDetailPage` (stats,
alerts, accounts, dispute-text export with copy-to-clipboard),
`AccountDetailPage` (single account + timeline). Auth state lives in
