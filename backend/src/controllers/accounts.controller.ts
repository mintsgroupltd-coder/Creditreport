...
export async function getAccountDetail(req: AuthenticatedRequest, res: Response) {
  ...
  res.json({
    account: {
      ...
      lenderContactAddress: account.lender.contactAddress,
    },
    timeline,
  });
}
...
const updateContactSchema = z.object({
  contactAddress: z.string().max(2000).nullable(),
});

/** Lets a user record the correspondence address they found for this
 * account's lender (from a statement, letter, or Experian's own report),
 * shared across every account for that same lender. We never fabricate
 * this ourselves — see the comment on Lender.contactAddress. */
export async function updateAccountLenderContact(req: AuthenticatedRequest, res: Response) {
  const account = await prisma.account.findFirst({
    where: { id: req.params.id, report: { userId: req.user!.id } },
  ...
  const { contactAddress } = updateContactSchema.parse(req.body);
  const trimmed = contactAddress?.trim() || null;

  await prisma.lender.update({
    where: { id: account.lenderId },
    data: { contactAddress: trimmed },
  });

  res.json({ contactAddress: trimmed });
}
