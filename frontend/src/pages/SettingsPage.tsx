...
export function SettingsPage() {
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      await api.updateProfile(fullName.trim() || null, postalAddress.trim() || null);
      setSaved(true);
  ...
  return (
    <AppShell>
      ...
      <p className="mt-1 text-sm text-slate-400">
        Used only to fill in the "[Your name]" / "[Your address]" placeholders in dispute letters, so they're ready to send without
        manual edits. Nothing else in the app reads this.
      </p>
